...
class TransformerSummarizer(nn.Module):
    ...
    def train_step(self, batch, optim):
        ...
        self.train()
        optim.zero_grad()
        probs, seq = self.forward(batch[0],batch[1])
        loss = self.criterion(probs.view(-1, self.vocab_size), batch[1].view(-1))
    ...
    def sample(self, src, max_seq_len=None):
        """Generate sample.
        ...
        with torch.no_grad():
            probs, seq = self.inference(src, max_target_seq_len=max_seq_len)
